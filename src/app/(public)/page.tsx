        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
            <div className="flex flex-col">
              <p className="text-sm font-semibold text-[#D4A853]">Watch the impact</p>
              <h2 className="font-heading mt-4 text-5xl font-black text-white md:text-6xl">
                A milestone —
                <br />
                <em className="not-italic text-[#D4A853]">15,000 meals</em>
                <br />
                and counting.
              </h2>
              <p className="mt-5 max-w-md text-lg leading-relaxed text-white/50">
                Every number has a face. Watch how one milestone captured what
                it means to nourish a student — and a community.
              </p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {[
                  { id: "jtfc82XdCn4", title: "When students speak, we listen", label: "Student voices" },
                  { id: "S80G83BsybQ", title: "Leadership in action — Chabot College", label: "On campus" },
                ].map((video) => (
                  <a
                    key={video.id}
                    ...
                    className="group relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.04] transition-all duration-300 hover:border-[#D4A853]/30 hover:bg-white/[0.07]"
                  >
                    <div className="relative aspect-video overflow-hidden">
                      <Image src={`https://i.ytimg.com/vi/${video.id}/hqdefault.jpg`} alt={video.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                      <div className="absolute inset-0 bg-[#0A1118]/40" />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow-lg transition-transform group-hover:scale-110">
                          <svg className="ml-0.5 h-4 w-4 text-[#1A3D5C]" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                        </div>
                      </div>
                    </div>
                    <div className="p-3">
                      <p className="text-xs font-semibold text-[#D4A853]">{video.label}</p>
                      <p className="mt-1 text-sm font-semibold leading-snug text-white/70 line-clamp-2">{video.title}</p>
                    </div>
                  </a>
                ))}
              </div>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-white/40 transition-colors hover:text-[#D4A853]"
              >
                View all videos
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[320px]">
                <div className="absolute -inset-4 rounded-3xl bg-[#D4A853]/10 blur-2xl" />
                <div className="relative overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl shadow-black/80" style={{ aspectRatio: "9/16" }}>
                  <iframe
                    src="https://www.youtube-nocookie.com/embed/7NLmjWjIpB8?rel=0&modestbranding=1"
